...
router.post('/forecast', async (req, res) => {
  try {
    const { product_id, store_id, forecast_days = 30, model_type = 'lstm' } = req.body;
...
router.get('/latest/:product_id/:store_id', async (req, res) => {
  try {
    ...
    const predictions = await sagemakerService.getLatestPredictions(product_id, store_id);
...
router.post('/batch/:store_id', async (req, res) => {
  try {
    ...
    const result = await sagemakerService.batchPredictStore(store_id, model_type);
...
router.get('/training-data/:product_id/:store_id', async (req, res) => {
  try {
    ...
    const data = await sagemakerService.getTrainingData(product_id, store_id, parseInt(days));
...
router.get('/features/:product_id/:store_id', async (req, res) => {
  try {
    ...
    const features = await sagemakerService.getProductFeatures(product_id, store_id);
